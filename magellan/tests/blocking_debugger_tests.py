...
def test_get_kgram():
	...
	features = [0, 1, 2, 3]
	expected_kgram_set = set(['abc', 'bc ', 'c 1', ' 12', '12 ', '2 1', '123', '234', '345', '456', '567', '678', '789', '890', '90 ', '0 e', ' en', 'end'])
	assert_equal(get_kgram(doc, features, 3), expected_kgram_set)
...
def test_debug_blocker_case_5():
	...
	expected_data = [[0, 21.0 / 32, 2, 3, 'John', 'John', 'Smith', 'Smith', '2345 Univ. Ave.', '2345 University Ave.'],
					 [1, 19.0 / 40, 3, 1, 'Kate', 'Katherine', 'Swift', 'Swift', '15 Regent Street, APT 10', '15 Regent St.'],
					 [2, 16.0 / 53, 1, 2, 'James', 'Jimmy', 'Doe', 'Doe', '1210 W. Dayton', 'Dept. of Comp. Sci., 1210 W. Dayton St.']]
	actual = debug_blocker(ltable, rtable, cand_set, 3)
	assert_equal(list(actual.columns), expected_columns)
	for i in range(len(actual)):
		assert_equal(list(actual.ix[i]), expected_data[i])
...
def test_debug_blocker_case_6():
	...
	expected_data = [[0, 13.0 / 23, 2, 3, 'John', 'John', 'Smith', 'Smith', '609-111-1111', '609,111,1111', '2345 Univ. Ave.', '2345 University Ave.'],
					 [1, 10.0 / 31, 1, 2, 'James', 'Jimmy', 'Doe', 'Doe', '(608)123-4567', '608-123-4567', '1210 W. Dayton', 'Dept. of Comp. Sci., 1210 W. Dayton St.'],
					 [2, 11.0 / 35, 3, 1, 'Kate', 'Katherine', 'Swift', 'Swift', '6086096100', '(608)609-6100', '15 Regent Street, APT 10', '15 Regent St.']]
	actual = debug_blocker(ltable, rtable, cand_set, 3, user_input_field_list)
	assert_equal(list(actual.columns), expected_columns)
	for i in range(len(actual)):
		assert_equal(list(actual.ix[i]), expected_data[i])
...
def test_debug_blocker_case_7():
	...
	assert_equal(debug_blocker(ltable, rtable, cand_set, 10, user_input_field_list).empty, False)
